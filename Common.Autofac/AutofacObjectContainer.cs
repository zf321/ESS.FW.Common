...
using ESS.FW.Common.Autofac.Impl;
...
namespace ESS.FW.Common.Autofac
{
    ...
    public class AutofacObjectContainer : IObjectContainer
    {
        ...
        public void RegisterType(Type implementationType, LifeStyle life = LifeStyle.Singleton)
        {
            ...
            SetMeta(implementationType, registrationBuilder);
            ...
            where TImplementer : class, TService
        {
            ...
            SetLifetimeScope(life, registrationBuilder);
            SetMeta(typeof(TImplementer), registrationBuilder);
            builder.Update(Container);
        }
            ...
            params string[] parameter)
        {
            ...
            SetMeta(implementer, registrationBuilder);
            SetLifetimeScope(life, registrationBuilder);
        ...
        protected virtual void SetMeta(Type type, IRegistrationBuilder<object, IConcreteActivatorData, SingleRegistrationStyle> registrationBuilder)
        {
            var meta = type.GetCustomAttributes().FirstOrDefault(c => c.GetType().GetCustomAttribute<MetadataAttributeAttribute>() != null);
            if (meta != null)
            {
                var props = meta.GetType().GetProperties();
                foreach (var p in props)
                {
                    if (p.Name == "TypeId") continue;
                    registrationBuilder.WithMetadata(p.Name, p.GetValue(meta));
                }
            }

        }
        protected virtual void SetMeta(Type type, IRegistrationBuilder<object, ReflectionActivatorData, DynamicRegistrationStyle> registrationBuilder)
        {
            var meta = type.GetCustomAttributes().FirstOrDefault(c => c.GetType().GetCustomAttribute<MetadataAttributeAttribute>() != null);
            if (meta != null)
            {
