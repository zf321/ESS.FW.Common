    ...
    public class IOHelper
    {
            ...
            int retryInterval = 1000) where TAsyncResult : AsyncTaskResult
        {
            var getContextInfo = new Func<string>(() =>
            {
                ...
                {
                    _logger.LogError("Failed to execute the getContextInfoFunc.", ex);
                    return null;
                }
            });
            var retryAction = new Action<int>(currentRetryTimes =>
            {
                ...
                {
                    _logger.LogError("Failed to execute the retryAction.", ex);
                }
            });
            var executeFailedAction = new Action<string>(errorMessage =>
            {
                ...
                {
                    _logger.LogError(string.Format("Failed to execute the failedCallbackAction of asyncAction:{0}, contextInfo:{1}", asyncActionName, getContextInfo()), ex);
                }
            });
            var processTaskException = new Action<Exception, int>((ex, currentRetryTimes) =>
            {
                ...
                {
                    _logger.LogError(string.Format("Async task '{0}' has io exception, contextInfo:{1}, current retryTimes:{2}, try to run the async task again.", asyncActionName, getContextInfo(), currentRetryTimes), ex);
                    retryAction(retryTimes);
                }
                else
                {
                    _logger.LogError(string.Format("Async task '{0}' has unknown exception, contextInfo:{1}, current retryTimes:{2}", asyncActionName, getContextInfo(), currentRetryTimes), ex);
                    if (retryWhenFailed)
                    {
                        retryAction(retryTimes);
                    }
            ...
            var completeAction = new Action<Task<TAsyncResult>>(t =>
            {
                try
                {
                    ...
                    if (t.IsCanceled)
                    {
                        _logger.LogError("Async task '{0}' was cancelled, contextInfo:{1}, current retryTimes:{2}.", asyncActionName, getContextInfo(), retryTimes);
                    ...
                    if (result == null)
                    {
                        _logger.LogError("Async task '{0}' result is null, contextInfo:{1}, current retryTimes:{2}", asyncActionName, getContextInfo(), retryTimes);
                        if (retryWhenFailed)
                        {
                            retryAction(retryTimes);
                        }
                    ...
                    {
                        _logger.LogError("Async task '{0}' result status is io exception, contextInfo:{1}, current retryTimes:{2}, errorMsg:{3}, try to run the async task again.", asyncActionName, getContextInfo(), retryTimes, result.ErrorMessage);
                        retryAction(retryTimes);
                    }
                    else if (result.Status == AsyncTaskStatus.Failed)
                    {
                        _logger.LogError("Async task '{0}' failed, contextInfo:{1}, current retryTimes:{2}, errorMsg:{3}", asyncActionName, getContextInfo(), retryTimes, result.ErrorMessage);
                        if (retryWhenFailed)
                        {
                            retryAction(retryTimes);
                        }
                ...
                {
                    _logger.LogError("Failed to execute the completeAction.", ex);
                }
            });
            ...
            {
                _logger.LogError(string.Format("IOException raised when executing async task '{0}', contextInfo:{1}, current retryTimes:{2}, try to execute the async task again.", asyncActionName, getContextInfo(), retryTimes), ex);
                retryAction(retryTimes);
            }
            catch (Exception ex)
            {
                _logger.LogError(string.Format("Unknown exception raised when executing async task '{0}', contextInfo:{1}, current retryTimes:{2}", asyncActionName, getContextInfo(), retryTimes), ex);
                if (retryWhenFailed)
                {
                    retryAction(retryTimes);
                }
